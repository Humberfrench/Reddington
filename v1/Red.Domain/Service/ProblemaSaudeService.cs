...
{
    public class ProblemaSaudeService : ServiceBase<ProblemaSaude>, IProblemaSaudeService
    {
        private readonly IProblemaSaudeRepository repository;
        private readonly ValidationResult validationResult = new ValidationResult();

        
        public ProblemaSaudeService(IProblemaSaudeRepository repository) : base(repository)
        {
            this.repository = repository;
        }
        public IEnumerable<ProblemaSaude> Filtrar(string nome)
        {
            var registros = Pesquisar(p => p.Descricao.ToLower().Contains(nome.ToLower())).ToList();
            return registros;
        }

        public ValidationResult Gravar(ProblemaSaude item)
        {
            if (item.Codigo == 0)
            {
                Adicionar(item);
